...
function normalize(a, b) {
  return (b -= a = +a) ? function(x) {
    return (x - a) / b;
  } : constant_default3(isNaN(b) ? NaN : 0.5);
}
...
function transformer() {
  ...
  function scale(x) {
    return isNaN(x = +x) ? unknown : (output || (output = piecewise2(domain.map(transform), range, interpolate)))(transform(clamp(x)));
  }
  scale.invert = function(y) {
    return clamp(untransform((input || (input = piecewise2(range, domain.map(transform), number_default2)))(y)));
  };
  scale.domain = function(_) {
    return arguments.length ? (domain = map3.call(_, number_default3), clamp === identity2 || (clamp = clamper(domain)), rescale()) : domain.slice();
  };
  scale.range = function(_) {
...
function tickFormat_default(start, stop, count, specifier) {
...
function linearish(scale) {
  ...
  scale.tickFormat = function(count, specifier) {
    var d = domain();
    return tickFormat_default(d[0], d[d.length - 1], count == null ? 10 : count, specifier);
  };
...
function identity3(domain) {
  ...
  domain = arguments.length ? map3.call(domain, number_default3) : [0, 1];
  return linearish(scale);
}
...
function quantile() {
  ...
  function scale(x) {
    return isNaN(x = +x) ? unknown : range[bisect_default(thresholds, x)];
  }
...
function quantize() {
  ...
  function scale(x) {
    return x <= x ? range[bisect_default(domain, x, 0, n)] : unknown;
  }
  function rescale() {
    var i = -1;
    domain = new Array(n);
...
function transformer2() {
  ...
  function scale(x) {
    return isNaN(x = +x) ? unknown : interpolator(k10 === 0 ? 0.5 : (x = (transform(x) - t03) * k10, clamp ? Math.max(0, Math.min(1, x)) : x));
  }
  scale.domain = function(_) {
    return arguments.length ? (t03 = transform(x0 = +_[0]), t13 = transform(x1 = +_[1]), k10 = t03 === t13 ? 0 : 1 / (t13 - t03), scale) : [x0, x1];
  };
  scale.clamp = function(_) {
...
function sequentialQuantile() {
  ...
  function scale(x) {
    if (!isNaN(x = +x))
      return interpolator((bisect_default(domain, x) - 1) / (domain.length - 1));
  }
  scale.domain = function(_) {
    if (!arguments.length)
      return domain.slice();
...
function transformer3() {
  var x0 = 0, x1 = 0.5, x2 = 1, t03, t13, t22, k10, k21, interpolator = identity2, transform, clamp = false, unknown;
  function scale(x) {
    return isNaN(x = +x) ? unknown : (x = 0.5 + ((x = +transform(x)) - t13) * (x < t13 ? k10 : k21), interpolator(clamp ? Math.max(0, Math.min(1, x)) : x));
  }
  scale.domain = function(_) {
    return arguments.length ? (t03 = transform(x0 = +_[0]), t13 = transform(x1 = +_[1]), t22 = transform(x2 = +_[2]), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), scale) : [x0, x1, x2];
  };
  scale.clamp = function(_) {
    return arguments.length ? (clamp = !!_, scale) : clamp;
  };
  ...
  return function(t) {
    transform = t, t03 = t(x0), t13 = t(x1), t22 = t(x2), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13);
    return scale;
  };
}
function diverging() {
